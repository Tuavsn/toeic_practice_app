      ...
      >
        ...
        <View className="flex-1 p-4 justify-between">
          ...
          <View className="flex-row justify-between items-center mt-2">
            <TouchableOpacity
              className={`px-3 py-2 rounded-lg flex-row items-center ${isSaved ? 'bg-blue-500' : 'bg-white bg-opacity-80'
                }`}
              onPress={onSave}
            >
              <Ionicons
              ...
              />
              <Text
                className={`text-sm font-medium ml-1 ${isSaved ? 'text-white' : 'text-blue-500'
                  }`}
              >
                {isSaved ? 'Đã lưu' : 'Lưu'}
              </Text>
